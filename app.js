...
async function loadSampleBackup() {
    ...
    Object.entries(backup).forEach(([key, value]) => {
        ...
        // サンプルは現行の見た目を常に示す。名簿・履歴はそのまま、色と任意の色分けのみ現行既定値へそろえる。
...
function updatePrintInactiveToggleUi() {
    ...
    btn.textContent = isFrame ? '空席 枠' : '空席 隠す';
    btn.setAttribute('aria-label', isFrame ? '空席を枠で表示中。非表示へ切り替えます' : '空席を非表示中。枠表示へ切り替えます');
...
function applyPrintOrientation() {
    ...
    if (title) title.textContent = '印刷プレビュー';
...
function initPrintSettingsPanel() {
    ...
    gender.innerHTML = `<div class="print-gender-settings"><div class="print-gender-active"><span>色分け</span><button id="ps-gb-active" class="btn btn-outline print-gender-toggle" type="button" aria-pressed="false">OFF</button><span>塗り</span><button id="ps-gb-fill" class="btn btn-outline print-gender-toggle" type="button" aria-pressed="false" disabled>OFF</button></div><div class="print-gender-controls"><label class="print-gender-field"><span>男</span><span class="print-gender-select"><select id="ps-gb-boy" data-gender-property="boy" aria-label="男子の枠線色">${[1,2,3,4,5,6].map(n => `<option value="${n}">${n}</option>`).join('')}</select></span></label><label class="print-gender-field"><span>女</span><span class="print-gender-select"><select id="ps-gb-girl" data-gender-property="girl" aria-label="女子の枠線色">${[1,2,3,4,5,6].map(n => `<option value="${n}">${n}</option>`).join('')}</select></span></label><label class="print-gender-field print-gender-style"><span>線</span><select id="ps-gb-style" data-gender-property="style" aria-label="色分けの線種"><option value="solid">通常</option><option value="thick">太線</option><option value="double">二重線</option></select></label></div></div>`;
    ...
    window.addEventListener('resize', () => closePrintColorPalette());
    syncPrintSettingsPanel(readSeatAppearanceSettingsFromMainUi());
}
...
function getColorControlForegroundColor(color) {
    ...
    const luminance = 0.2126 * toLinear(hex.slice(1, 3)) + 0.7152 * toLinear(hex.slice(3, 5)) + 0.0722 * toLinear(hex.slice(5, 7));
...
function refreshPrintColorControls() {
    if (!document.getElementById('print-settings-panel')) return;
    syncPrintSettingsPanel(readSeatAppearanceSettingsFromMainUi());
}
...
function openPrintColorPalette(index, anchor) {
    ...
    palette.setAttribute('aria-label', `色${index}を選択`);
    ...
    positionPrintColorPalette(palette, anchor);
    anchor?.setAttribute('aria-expanded', 'true');
...
function positionPrintColorPalette(palette, anchor) {
    if (!palette || !anchor) return;
    const margin = 8;
    const gap = 8;
    const width = Math.min(360, Math.max(280, window.innerWidth - margin * 2));
    palette.style.width = `${width}px`;
    palette.style.left = `${margin}px`;
    palette.style.top = `${margin}px`;
    const anchorRect = anchor.getBoundingClientRect();
    const paletteRect = palette.getBoundingClientRect();
    const left = Math.min(Math.max(margin, anchorRect.left), window.innerWidth - paletteRect.width - margin);
    let top = anchorRect.bottom + gap;
    if (top + paletteRect.height > window.innerHeight - margin) {
        top = Math.max(margin, anchorRect.top - paletteRect.height - gap);
    }
...
function closePrintColorPalette(returnFocus = false) {
    document.querySelectorAll('.print-color-palette:not([hidden])').forEach(palette => {
        palette.hidden = true;
        palette.innerHTML = '';
        palette.style.width = '';
        palette.style.left = '';
        palette.style.top = '';
        delete palette.dataset.index;
    });
